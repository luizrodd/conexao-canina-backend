namespace ConexaoCaninaApp.Domain.Models
{
    ...
    public class Cao
    {
        ...
        public Cao(string cidade, string estado, string nome, 
            string description, string raca, int idade, TamanhoCao tamanho,
            GeneroCao genero, string caracteristicasUnicas, List<Foto> fotos)
        {
            CaoId = Guid.NewGuid();
            Cidade = cidade;
            Estado = estado;
        ...
        public ICollection<Foto> Fotos { get; set; } = new List<Foto>(); //{userid}/caes/{caoid}/{fotoid}
